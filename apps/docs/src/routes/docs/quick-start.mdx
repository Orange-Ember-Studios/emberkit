...
<Alert variant="info" title="How it works">
  The <code>span</code> renders with the initial count value in the HTML. 
  When the button is clicked, <code>setCount</code> updates the signal, and only the <code>span</code>'s <code>textContent</code> changes — no re-render of the whole component.
</Alert>
