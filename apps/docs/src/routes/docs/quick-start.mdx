...
<Alert variant="info" title="How it works">
  SSR writes the initial count into HTML with <code>data-ek-bind</code>. After load, only the bound <code>span</code> updates when the signal changes. See [Hydration](/docs/hydration) for all binding attributes.
</Alert>
...
Or use `<Head>` from `@emberkit/core` inside components — see [Head](/docs/head).
...
Set `mode: 'static'` in `emberkit.config.ts` for a fully static export, or `hybrid` (default) for static pages plus SSR for dynamic routes. See [SSR & SSG](/docs/ssr).

## What's Next?

- [Routing](/docs/routing) — Layouts, loaders, API routes, error pages
- [SSR & SSG](/docs/ssr) — Rendering modes and build pipeline
- [Signals](/docs/signals) — Reactive state
- [Components](/docs/components) — Static vs interactive UI
