...
<Alert variant="info" title="Key Concept">
  <strong>Static components</strong> (no event handlers, no signal bindings) render to pure HTML with zero JavaScript.<br />
  <strong>Interactive components</strong> (with <code>onClick</code>, <code>onChange</code>, etc. or <code>data-ek-bind</code>) receive targeted client-side hydration.
</Alert>
