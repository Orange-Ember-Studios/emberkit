...
<Text color="muted" className="mb-6">Programmatic meta tag and JSON-LD generation for servers, scripts, and templates. For declarative <code className="text-primary-400">{'<head>'}</code> tags in components, use the <code className="text-primary-400">{'<Head>'}</code> component — documented in [Head](/en/docs/head).</Text>

<Alert variant="info" title="Route metadata">
  The default SSR pipeline also supports <code>export const metadata = {'{ title, description }'}</code> on route modules. See [SSR & SSG](/en/docs/ssr#route-metadata-built-in-ssr-head).
</Alert>
...
- [Head](/en/docs/head) — Declarative `<Head>` in JSX
- [SSR & SSG](/en/docs/ssr) — Built-in `metadata` export and SSR pipeline
- [Markdown/MDX](/en/docs/markdown) — Frontmatter for content pages
