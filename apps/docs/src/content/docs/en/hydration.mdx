...
<Alert variant="info" title="Different from image lazy loading">
  <code>LazyInView</code> delays <strong>component HTML and client work</strong> for a section. Image <code>loading="lazy"</code> only defers image requests — see [Image Optimization](/en/docs/images).
</Alert>
...
**3. SSR loaders** — fetch list data in a route <code>loader</code> so the server HTML includes items. See [SSR & SSG](/en/docs/ssr).
...
- [Signals](/en/docs/signals) — Signal API and subscribe
- [SSR](/en/docs/ssr) — Server-side rendering
- [Release 0.8.0](/en/docs/release-0-8) — Dev API, view transitions, hydration notes
- [Components](/en/docs/components) — Static vs interactive components
- [API Reference](/en/docs/api) — `LazyInView`, `hydrateLazyInView`, `clearLazyRegistry`
